 ...
 */
	...
	class DW_Single extends DWModule {
		protected static $info = 'Khi bạn chọn ngoại trừ Tác Giả <b>VÀ</b> Chuyên Mục, thì cả hai điều kiện phải đồng thời thỏa. Nếu không luật ngoại trừ sẽ không được chấp nhận. Nếu bạn bạn muốn theo luật <b>OR</b> thì hãy thêm một widget y hệt, và thiết lập luật loại trừ còn lại cho nó'; // 'When you use an author <b>AND</b> a category exception, both rules in the condition must be met. Otherwise the exception rule won\'t be applied. If you want to use the rules in a logical OR condition. Add the same widget again and apply the other rule to that.';
		public static $option = array( 'single' => 'Single Posts' );
		protected static $question = 'Hiển thị widget trên trang chi tiết bài viết'; // 'Show widget default on single posts?';
	...
	foreach ( $tax_list as $tax_type ) {
		if ( $tax_type->name != 'post_tag' && $tax_type->name != 'category' ) {
			...
			if ( count($tax) > 0 ) {
				...
				echo __('Ngoại trừ', DW_L10N_DOMAIN) . ' ' . $tax_type->label . ':<br />';
