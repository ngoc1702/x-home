 ...
 */
	...
	class DW_Category extends DWModule {
		private static $except = 'Ngoại trừ chuyên mục';
		public static $option = array( 'category' => 'Category Pages' );
		protected static $question =  'Hiển thị widget trên trang chuyên mục?'; //'Show widget default on category pages?';
		...
		public static function prtCat($categories, $category_act, $category_childs_act, $single = FALSE) {
			...
			foreach ( $categories as $pid => $childs ) {
				...
				if ( $run ) {
					...
					echo '<div style="position:relative;left:15px;">';
					echo '<input type="checkbox" id="' . ( $single ? 'single_' : '' ) . 'category_childs_act_' . $cat->cat_ID . '" name="' . ( $single ? 'single_' : '' ) . 'category_childs_act[]" value="' . $cat->cat_ID . '" ' . ( isset($category_childs_act) && count($category_childs_act) > 0 && in_array($cat->cat_ID, $category_childs_act) ? 'checked="checked"' : '' ) . ' onchange="chkParent(\'' . ( $single ? 'single_' : '' ) . 'category\', ' . $cat->cat_ID . ');' . ( $single ? 'ci(\'single_category_act_' . $cat->cat_ID . '\')' : '' ) . '" /> <label for="' . ( $single ? 'single_' : '' ) . 'category_childs_act_' . $cat->cat_ID . '"><em>' . __('Tất cả con', DW_L10N_DOMAIN) . '</em></label><br />';
					echo '</div>';
