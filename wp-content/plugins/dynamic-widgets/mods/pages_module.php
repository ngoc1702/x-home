 ...
 */
	...
	class DW_Page extends DWModule {
		...
		protected static $question = 'Hiển thị widget trên trang tĩnh?'; // 'Show widget default on static pages?';
		...
		public static function admin() {
			...
			if ( $num_pages > 0 ) {
				...
				echo '<br />';
				_e('Ngoại trừ trang', DW_L10N_DOMAIN);
				echo '<br />';
		...
		public static function infoText() {
			return __('Chọn "Tất cả con", gây hiệu lực tới trang được chọn cùng tất cả con cháu của nó, cả hiện tại và tương lai. Không thể lựa chọn luật chỉ riêng cho con cháu mà bỏ qua trang đang xét.', DW_L10N_DOMAIN);
		...
		function start_el(&$output, $page, $depth = 0, $args = array(), $current_object_id = 0) {
			...
			if ( $page->ID <> $this->post_page ) {
				...
				if ( $args['depth'] > -1 ) {
					...
					$output .= '<input type="checkbox" id="page_childs_act_' . $page->ID . '" name="page_childs_act[]" value="' . $page->ID . '" ' . ( isset(DW_Page::$opt_page_childs->act) && count(DW_Page::$opt_page_childs->act) > 0 && in_array($page->ID, DW_Page::$opt_page_childs->act) ? 'checked="checked"' : '' ) . ' onchange="chkParent(\'page\', ' . $page->ID . ')" /> <label for="page_childs_act_' . $page->ID . '"><em>' . __('Tất cả con', DW_L10N_DOMAIN) . '</em></div>';
				}
