<?php
/**
 * Plugin Name: Dynamic Widgets - Caia
...
**/
	...
	define('DW_OLD_METHOD', false);
	define('DW_PAGE_LIMIT', 1);
   ...
   */
	...
	*/
	function dynwid_add_admin_menu() {
		...
		$dw_admin_screen = add_submenu_page('themes.php', __('Tùy biến Widgets', DW_L10N_DOMAIN), __('Tùy biến Widgets', DW_L10N_DOMAIN), 'edit_theme_options', 'dynwid-config', 'dynwid_admin_page');
	...
	*/
	...
	function dynwid_widget_callback() {
		...
		echo '<p>' . __('Tùy biến Widgets', DW_L10N_DOMAIN) . ': ';

		if ( array_key_exists($widget_id, $DW->registered_widgets) ) {
			$widget_id_opt = $DW->hasOptions($widget_id);
			...
			echo ( $widget_id_opt ) ? __('Dynamic', DW_L10N_DOMAIN) : __('Static', DW_L10N_DOMAIN);
			echo '</a>';
			
			if ( $widget_id_opt ) {
				$s = array();
				$arr = array();
				$opt = $DW->get_types($widget_id);
				...
				if ( count($opt) > 0 ){
					$output  = '<br /><small>';
					$output .= __('Thiết lập cho:', DW_L10N_DOMAIN);
					$output .= ' ' . $string . '.</small>';
					echo $output;
				}
