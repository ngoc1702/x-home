 ...
 */
	...
	class dynWid {
		...
		public  $whereami;
		private $wpdb;
		private $dwList_results;
		private $dwid_type_list;
		...
		public function __construct() {
			...
			$query = "select 1 from " . $this->dbtable . " limit 1";
			$result = $this->wpdb->get_var($query);
			// echo $query;
			// print_r($result);

			$this->enabled = $result === FALSE ? FALSE : TRUE;
		...
		public function dwList($whereami) {
			...
			foreach ( $results as $myrow ) {
				if (!in_array($myrow->widget_id, $this->dynwid_list)){
		...
		public function getDWOpt($widget_id, $maintype) {
			...
			// Caia
			$query = "SELECT widget_id, maintype, name, value FROM " . $this->dbtable . "
                 WHERE widget_id = '" . esc_sql($widget_id) . "'
		...
		public function getOpt($widget_id, $maintype, $admin = TRUE) {
			...
			if ( $admin ) {
				...
				if (isset($this->dwList_results)){
					$results = array();
					foreach ($this->dwList_results as $key => $myrow) {
						if ($myrow->widget_id == $widget_id){
							$results[] = $myrow;
						}
					}
				}else{									
		...
		public function hasOptions($widget_id) {
			...
			// Caia
			if (!isset($this->dwid_type_list)){
				// $query = "SELECT distinct widget_id FROM " . $this->dbtable . " WHERE maintype != 'individual'";
				// // $query = $this->wpdb->prepare($query, $widget_id, 'individual');
				// $this->dwid_list = $this->wpdb->get_col($query);
				$this->make_dwid_type_list();
			}
					
			return isset($this->dwid_type_list[$widget_id]);
		}

		// caia
		public function get_types($widget_id){
			if (!isset($this->dwid_type_list)){				
				$this->make_dwid_type_list();
			}
					
			return $this->dwid_type_list[$widget_id];
		}


		// Caia
		private function make_dwid_type_list(){
			if (!isset($this->dwid_type_list)){
				$query = "SELECT distinct widget_id, maintype FROM " . $this->dbtable . " WHERE maintype != 'individual'";
				// $query = $this->wpdb->prepare($query, $widget_id, 'individual');
				$results = $this->wpdb->get_results($query, ARRAY_A);
				// print_r($results);
				$arr = array();
				foreach ($results as $key => $row) {
					$arr[$row['widget_id']][] = $row['maintype'];									
				}
				$this->dwid_type_list = $arr;
			}
		}
