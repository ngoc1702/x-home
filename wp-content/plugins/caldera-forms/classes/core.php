...
class Caldera_Forms {
	...
	public static function activate_caldera_forms( $force = false ){
		if ( ! wp_next_scheduled( 'caldera_forms_tracking_send_rows' ) ) {
			...
			foreach($magics[1] as $magic_key=>$magic_tag){
				...
				if(count($magic) == 2){
					switch (strtolower( $magic[0]) ) {
						...
						case 'cookie':
							if ($magic[1] === 'source'){
								$magic_tag = '~cookie:source~';							
							} else if ($magic[1] === 'landing_page'){
								$magic_tag = '~cookie:landing_page~';
							} else {
								$magic_tag = null;
							}
							break;
	...
	static public function process_submission(){
		...
		if( empty( $transdata['edit'] ) && !empty($form['db_support']) ){
			...
			$new_entry = array(
				'form_id'	=>	$form['ID'],
				'user_id'	=>	0,
				'datestamp' =>	date_i18n( 'Y-m-d H:i:s', time(), 1),
				'status'	=>	'pending'
			);
			// if user logged in
			if(is_user_logged_in()){
